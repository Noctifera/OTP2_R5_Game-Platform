...
public class Ghost implements Ghost_IF {
	...
	/*
	 * public void testPath(Point target, ArrayList<Point> list) { //int dX = (int)
	 * (target.getX() - list.get(list.size() - 1).getX()); //int dY = (int)
	 * (target.getY() - list.get(list.size() - 1).getY());
	 * 
	 * //boolean suppress = false; ArrayList<Point> possibleSpaces =
	 * ml.freeSpaces(); int possible = 0; String string = ""; // ylös if
	 * (possibleSpaces.contains(up(list.get(list.size() - 1))) &&
	 * !list.contains(up(list.get(list.size() - 1)))) {
	 * System.out.println("possible up"); possible++; string = string + " up";
	 * System.out.println("string" + string); } // alas if
	 * (possibleSpaces.contains(down(list.get(list.size() - 1))) &&
	 * !list.contains(down(list.get(list.size() - 1)))) {
	 * System.out.println("possible down"); possible++; string = string + " down";
	 * System.out.println("string" + string); } // vasemalle if
	 * (possibleSpaces.contains(left(list.get(list.size() - 1))) &&
	 * !list.contains(left(list.get(list.size() - 1)))) {
	 * System.out.println("possible left"); possible++; string = string + " left";
	 * System.out.println("string" + string); } // oikealle if
	 * (possibleSpaces.contains(right(list.get(list.size() - 1))) &&
	 * !list.contains(right(list.get(list.size() - 1)))) {
	 * System.out.println("possible right"); possible++; string = string + " right";
	 * System.out.println("string" + string); }
	 * 
	 * if (possible == 1) { if (string.contains("up")) {
	 * System.out.println("one up"); list.add(up(list.get(list.size() - 1)));
	 * 
	 * } if (string.contains("down")) { System.out.println("one down");
	 * list.add(down(list.get(list.size() - 1)));
	 * 
	 * } if (string.contains("left")) { System.out.println("one left");
	 * list.add(left(list.get(list.size() - 1))); } if (string.contains("right")) {
	 * System.out.println("one right"); list.add(right(list.get(list.size() - 1)));
	 * } testPath(target, list);
	 * 
	 * } else if (possible > 1) { double random = Math.random(); if
	 * (string.contains("up") && random <= 0.25) {
	 * System.out.println("multiple up"); ArrayList<Point> up = new ArrayList<>();
	 * up.addAll(list); up.add(new Point(up(list.get(list.size() - 1)))); gh2 = new
	 * Ghost_Thread2(this, up, target); gh2.start();
	 * 
	 * } if (string.contains("down")&& (0.25 < random && random <= 0.5 )) {
	 * System.out.println("multiple down"); ArrayList<Point> down = new
	 * ArrayList<>(); down.addAll(list); down.add(new
	 * Point(down(list.get(list.size() - 1)))); gh2 = new Ghost_Thread2(this, down,
	 * target); gh2.start();
	 * 
	 * } if (string.contains("left") && (random > 0.5 && 0.75>=random)) {
	 * System.out.println("multiple left"); ArrayList<Point> left = new
	 * ArrayList<>(); left.addAll(list); left.add(new
	 * Point(left(list.get(list.size() - 1)))); gh2 = new Ghost_Thread2(this, left,
	 * target); gh2.start();
	 * 
	 * } if (string.contains("right") && (random <=1 && random >0.75)) {
	 * System.out.println("multiple right"); ArrayList<Point> right = new
	 * ArrayList<>(); right.addAll(list); right.add(new
	 * Point(right(right.get(right.size() - 1)))); gh2 = new Ghost_Thread2(this,
	 * right, target); gh2.start(); } } //System.out.println("target:" + target +
	 * " , " + "position: " + list.get(list.size() - 1)); //
	 * System.out.println("path: " + list.toString());
	 * if(list.get(list.size()-1).equals(target)) { path = list;
	 * System.out.println("path found"); System.out.println("path: "+path);
